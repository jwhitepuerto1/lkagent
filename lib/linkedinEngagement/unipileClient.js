...
// as a rough cap, not exact. Fixed to network_distance:[1] (1st-degree
// only) and api:"classic" since that's the only mode this one-off
// search-and-export tool needs.
export async function searchPeople({ keywords, limit = 50, cursor } = {}) {
  if (!keywords) throw new Error("keywords is required");
  const data = await unipilePost("/linkedin/search", {
    params: { limit, cursor },
    body: { api: "classic", category: "people", keywords, network_distance: [1] },
  });
